...
function resolveAssistantConfig(env = process.env) {
  const memoryVersion = normalizeMemoryVersion(env.TENCENT_MEMORY_API_VERSION || env.TENCENTDB_MEMORY_API_VERSION);
  const rawMemoryEndpoint = env.TENCENT_MEMORY_ENDPOINT || env.TENCENTDB_MEMORY_ENDPOINT || '';
  const memoryEndpoint = normalizeMemoryEndpoint(rawMemoryEndpoint, memoryVersion);
  const memoryApiKey = trim(env.TENCENT_MEMORY_API_KEY || env.TENCENTDB_MEMORY_API_KEY);
  const memoryServiceId = trim(env.TENCENT_MEMORY_SERVICE_ID || env.TENCENTDB_MEMORY_SERVICE_ID);
  const memoryTeamId = trim(env.TENCENT_MEMORY_TEAM_ID || env.TENCENTDB_MEMORY_TEAM_ID) || 'crmzona';
  const memoryAgentId = trim(env.TENCENT_MEMORY_AGENT_ID || env.TENCENTDB_MEMORY_AGENT_ID) || 'crmzona-assistant';
  ...
  const openAi = {
    ...
    model: trim(env.OPENAI_MODEL || env.AI_ASSISTANT_MODEL) || DEFAULT_OPENAI_MODEL,
...
async function callOpenAiResponse(openAiConfig, prompt, fetchImpl, user) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new AssistantRequestError(
        response.status,
        openAiErrorMessage(payload) || `OpenAI API вернул HTTP ${response.status}.`,
        openAiErrorDetails(payload),
      );
    }
    const text = extractOpenAiText(payload);
    if (!text) {
      throw new AssistantRequestError(502, 'OpenAI API вернул пустой ответ.');
    }
    return text;
  } catch (error) {
